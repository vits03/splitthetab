...
const MyDocument = ({ formData = {}, buyers = [] }) => {
 ...
 const calculateTotalSpendings=()=>{
  ...
  // iterate though buyer.descriptions
  //add all description.amount
  const sum =buyers.reduce ((total,buyer)=>{
    return total+buyer.descriptions.reduce((total,description)=>{
  ...
  const OwedSection = ({ buyers, person }) => {
    ...
    buyers.forEach((buyer) => {
      buyer.descriptions.forEach((descriptionObj) => {
        if (descriptionObj.contributors.includes(person)) {
          ...
          const owed =
            descriptionObj.amount / descriptionObj.contributors.length;
    ...
    return (
      <View
        style={{ border: "2px solid black", borderRadius: 20, padding: 15 }}
      >
        <View
          style={{
            display: "flex",
        ...
        >
          ...
          />
          <Text style={{color:"#E34234"}}>{capitalizeFirstLetter(person)}</Text>
        </View>
        <View>
          <Text>owes:</Text>
          {calculatedMembers.length > 0 ? (
            calculatedMembers.map((member, index) => (
              ...
              >
                ...
                <Text>to </Text>
                <Text style={{ fontWeight: "bold" }}>{ capitalizeFirstLetter(member.name)}</Text>
              </View>
            ))
          ) : (
  ...
  const ExpenseTable = ({ Buyers }) =>
    ...
    <Text  style={{ display:"inline-block",fontWeight:'bold',marginBottom:2 }}key={index}>{contributor},</Text>
  ))}
              </View>
        ...
        <Text
          ...
          <Text style={{ marginHorizontal: 4, fontWeight: "bold",fontSize:15,color:"#E34234" }}>
            Rs{" "}
  ...
  const styles = StyleSheet.create({
    ...
    section: {
      backgroundColor:'#E34234', // Some PDF renders may support
      paddingTop: 10,
      ...
      <View style={styles.team}>
        {Members.map((teamMember, index) => (
          <View key={index} style={styles.teamMember}>
            <Image src={userIcons} style={{ width: 22, height: 22,borderRadius:35 ,backgroundColor:'#E97451'}} />
    ...
    <Document >
      <Page size="A4" style={styles.page} wrap={false}>
        <View style={styles.section}>
          <Text style={styles.header}>{capitalizeFirstLetter(formData.partyName) || "partyname"}</Text>
          <Text style={styles.date}>{formData.date}</Text>
        </View>

        <View style={styles.teamSection}>
          <Text
            style={{ fontSize: 25, marginVertical: 35, fontWeight: "bold", color: "#E34234" }}
          >
            Lekip
          </Text>
       ...
       <View style={{flexBasis:"70%",marginTop:25}}>
            <Text style={{fontWeight:'bold',fontSize:20}}>  Total Expense: Rs <Text style={{color:"#E34234"}}>{calculateTotalSpendings() || 0}</Text></Text>
